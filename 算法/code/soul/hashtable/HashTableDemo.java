...
class EmpLinkedList{
	...
	// 添加雇员到链表
	// 1 假设当添加雇员 时 id 是自增长  即id 的分配总是从小到大 
	...
	public void add(Emp emp ) {
		...
		// 如果不是第一个雇员 则使用一个辅助的节点  帮助定位到最后的
		Emp curEmp = head;
		while(true) {
			if(curEmp.next == null) {// 如果到链表最后
				break;
			}
			curEmp = curEmp.next ;//往后移
		}
		// 退出while 循环时直接将emp 加入链表
		curEmp.next = emp;
	}
	...
	public void list(int no ) {
		...
		while(true) {
			...
			if(curEmp.next == null) {// 说明curEmp 已经是最后的节点
				break;
			}
